    ...
    const graphProto = {
        ...
        routeBetween(startCoords, endCoords) {
            ...
            // loop
            while(open.length > 0) {
                // set current to be the node with the lowest f_cost
                let indexOfLowest = 0
                ...
                const current = open[indexOfLowest]
                closed.push(open.splice(indexOfLowest, 1))
                ...
                if(current.node === endNode) {
                    ...
                    while(next) {
                        route.coordinates.unshift(next.node.geometry.coordinates)
                        next = next.parent
                    }
                    return route
                }
                ...
                for(const connection of current.node.connections) {
                    // if neighbour is in closed, skip to the next neighbour
